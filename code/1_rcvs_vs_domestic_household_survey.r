...
# Rename the varibles
survey_summary_stat[variable == "age",variable := "Возраст, лет"]
survey_summary_stat[variable == "agegroup_18-34",variable := "18-34 лет, %"]
survey_summary_stat[variable == "agegroup_35-49",variable := "35-49 лет, %"]
survey_summary_stat[variable == "agegroup_50-64",variable := "50-64 лет, %"]
survey_summary_stat[variable == "agegroup_65+",variable := "65+ лет, %"]
survey_summary_stat[variable == "crimetype_Assault",variable := "Нападение"]
survey_summary_stat[variable == "rimetype_Fraud",variable := "Мошенничество"]
survey_summary_stat[variable == "crimetype_Theft",variable := "Кража"]
survey_summary_stat[variable == "crimetype_Larceny",variable := "Мелкая кража"]
survey_summary_stat[variable == "crimetype_Other",variable := "Другое преступление"]
survey_summary_stat[variable == "crimetype_Fraud",variable := "Мошенничество"]
survey_summary_stat[variable == "crimetype_Wire Fraud/Cybercrime",variable := "Удаленное преступление"]
survey_summary_stat[variable == "crimetype_Attempted Wire Fraud/Cybercrime",variable := "Покушение на удаленное преступление"]
survey_summary_stat[variable == "crimetype_Robbery",variable := "Грабеж и разбой"]
survey_summary_stat[variable == "crimetype_Theft",variable := "Кража"]
survey_summary_stat[variable == "education_level_1",variable := "Полное среднее и ниже"]
survey_summary_stat[variable == "education_level_2",variable := "Среднее специальное"]
survey_summary_stat[variable == "education_level_3",variable := "Высшее и незаконченное высшее"]
survey_summary_stat[variable == "fears_becoming_victim",variable := "Боится стать жертвой преступления"]
survey_summary_stat[variable == "household_size",variable := "Размер домохозяйства, чел."]
survey_summary_stat[variable == "income_level_1",variable := "Не хватает на продукты, %"]
survey_summary_stat[variable == "income_level_2",variable := "Не хватает на одежду, %"]
survey_summary_stat[variable == "income_level_3",variable := "Не хватает на технику и мебель, %"]
survey_summary_stat[variable == "income_level_4",variable := "На технику хватает, на большее нет, %"]
survey_summary_stat[variable == "income_level_5",variable := "Можем позволить себе автомобиль, %"]
survey_summary_stat[variable == "income_level_6",variable := "Можем позволить себе недвижимость, %"]
survey_summary_stat[variable == "is_rural",variable := "Живет в сельской местности"]
survey_summary_stat[variable == "is_unsafe_alone",variable := "В темное время не ощущает себя в безопасности"]
survey_summary_stat[variable == "is_unsafe_at_home",variable := "Опасается ограбления своей квартиры/дома"]
survey_summary_stat[variable == "lives_alone",variable := "Живет один/одна"]
survey_summary_stat[variable == "male",variable := "Мужчина"]
survey_summary_stat[variable == "married",variable := "Женат/Замужем"]
survey_summary_stat[variable == "mean_household_income",variable := "Ежемесячный доход на члена домохозяйства, тыс. руб."]
survey_summary_stat[variable == "life_satisfaction",variable := "Удовлетворенность жизнью (от 1 до 10)"]
survey_summary_stat[variable == "unemployed",variable := "Безработный"]
survey_summary_stat[variable == "locality_avg_income",variable := "Среднедушевой месячный доход в местности, тыс. руб."]
survey_summary_stat[variable == "locality_population",variable := "Население местности, тыс. чел."]
survey_summary_stat[variable == "victimized12m",variable := "Был жертвой преступления за последние 12 мес."]
survey_summary_stat[variable == "victimized5y",variable := "Был жертвой преступления за последние 5 лет"]
